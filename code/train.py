...
def eval(model, iterator, f, criterion, binary_criterion):
    ...
    if num_task != 2:
        for i in range(num_task):
            ...
            num_correct += (np.logical_and(y_true[i] == y_pred[i], y_true[i]>1)).astype(np.int64).sum()
            num_gold += len(y_true[i][y_true[i]>1])
    elif num_task == 2:  
        num_predicted += len(y_pred[0][y_pred[0] > 1])
        num_correct += (np.logical_and(y_true[0] == y_pred[0], y_true[0]>1)).astype(np.int64).sum()
