...
def calc_unweighted_article_prop_den(article_length, article_word_count, article_annotations):
    length_scores = []
    count_scores = []
    ...
    for a in article_annotations:
        ...
        apdl = ann_length / int(article_length)
        apdt = a['word_count'] / int(article_word_count)
        length_scores.append(apdl)
        count_scores.append(apdt)

    return sum(length_scores), sum(count_scores)
...
# def count_prop_words_per_article(filename):
...
#
#             line = line.strip()
#             # Check if the line is an article id
#             if line.strip().isdigit():
#                 current_article_id = line.strip()
#                 if current_article_id not in article_counts:
#                     article_counts[current_article_id] = 0
#
#             # Split the line to get the word and its annotation
#             parts = line.split()
#             if len(annotation) == 3:
#                 _, _, annotation = parts
#                 if annotation == 'Prop':
#                     article_counts[current_article_id] += 1
#
